...
Canonical: `npm run lint`, `node --test dist/tests/canonical-json-profile-inventory.test.js`, and `node --test tests/canonical-json-runtime-parity.test.mjs`. Full repository `npm test` was exercised but is not a PAN433 gate: the local environment has no Docker executable/daemon and the repository-wide run also hit temporary-space exhaustion in a supply-chain fixture copy. The exact observed failures are recorded in `/tmp/pan433-full-test-failures.log`; no provider/runtime files were changed.
