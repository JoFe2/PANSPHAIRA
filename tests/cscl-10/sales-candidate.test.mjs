...
import { buildSalesCandidate, canonicalJson, deriveSalesCandidate, renderArtifacts } from "../../src/cscl-10/sales-candidate.mjs";

const root = resolve(import.meta.dirname, "../..");

test("binds the released CSCL-07 authority and exact sales denominator", async () => {
  const result = await buildSalesCandidate({ repoRoot: root });
  ...
  assert.equal(result.candidate.capabilityFamily, "SALES_ORDER_MANAGEMENT");
...
test("preserves every sales classification, matrix cell, fact, and original evidence reference", async () => {
  const { candidate } = await buildSalesCandidate({ repoRoot: root });
  ...
  assert.deepEqual(candidate.relationCounts, { UNRESOLVED: 10, VARIANT_RELATION: 1, DENIED: 1 });
  ...
  assert.equal(candidate.systemVariants.length, 53);
  assert.equal(candidate.absences.length, 1);
  assert.equal(candidate.unresolvedConflicts.length, 6);

  assert.equal(candidate.promotedVariantRelations.length, 1);
  const promoted = candidate.promotedVariantRelations[0];
  assert.equal(promoted.relationId, "relation:SALES_ORDER_MANAGEMENT:states-transitions");
  assert.equal(promoted.questionId, "states-transitions");
  assert.equal(promoted.state, "VARIANT_RELATION");
  assert.equal(promoted.reason, "FROZEN_EXACT_PURPOSE_AND_DIFFERING_OPERATIONAL_DIMENSION_EDGE");
  assert.deepEqual(promoted.supportedDimensions, ["purpose", "statesTransitions"]);
  assert.equal(promoted.cellReferences.length, 5);
  assert.equal(promoted.notPromotedToCore, true);
  assert.deepEqual(promoted.cellReferences.map((cell) => cell.systemId), [
    "odoo-community", "erpnext", "dolibarr", "tryton", "apache-ofbiz",
  ]);
  assert.ok(promoted.cellReferences.every((cell) =>
    /^[a-f0-9]{64}$/.test(cell.sourceCellDigest) && /^[a-f0-9]{64}$/.test(cell.sourceProfileDigest) &&
      cell.sourceFactIds.length > 0 && cell.evidenceReferences.length > 0
  ));
  assert.ok(promoted.cellReferences.flatMap((cell) => cell.evidenceReferences).every((reference) =>
    reference.exactLocator && /^[a-f0-9]{64}$/.test(reference.excerptSha256) && reference.sourceFactId
  ));

  assert.match(candidate.completeEvidenceConclusion, /states-transitions VARIANT_RELATION/);
  assert.match(candidate.completeEvidenceConclusion, /10 UNRESOLVED and 1 DENIED/);
});

test("replays independently and denies caller, frequency, extra-field, holdout, and authority mutations", async () => {
  const first = await buildSalesCandidate({ repoRoot: root });
  const second = await buildSalesCandidate({ repoRoot: root });
  ...
  assert.equal(canonicalJson(deriveSalesCandidate(matrix)), canonicalJson(first.candidate));
  assert.equal(matrix.relationCandidates.filter((relation) => relation.state === "VARIANT_RELATION").length, 2);
  assert.equal(matrix.relationCandidates.filter((relation) =>
    relation.capabilityFamily === "SALES_ORDER_MANAGEMENT" && relation.state === "VARIANT_RELATION"
  ).length, 1);
  assert.equal(matrix.relationCandidates.filter((relation) => relation.state === "POTENTIAL_EQUIVALENCE").length, 0);
  ...
  const attacks = [
    ...
    ["sales relation promoted to equivalence", (changed) => {
      const salesRelation = changed.relationCandidates.find(
        (relation) => relation.capabilityFamily === "SALES_ORDER_MANAGEMENT" && relation.state === "UNRESOLVED"
      );
      salesRelation.state = "POTENTIAL_EQUIVALENCE";
    }],
  ];
  for (const [label, mutate] of attacks) {
    const changed = structuredClone(matrix);
    mutate(changed);
    assert.throws(() => deriveSalesCandidate(changed), /SOURCE_MATRIX_DIGEST_MISMATCH/, label);
  }
});
...
test("freezes deterministic candidate, receipt, and Mapping/Application Guide artifacts", async () => {
  ...
  assert.equal(first.candidate, await readFile(resolve(root, "verification/cscl-10-sales-candidate-v1.json"), "utf8"));
  assert.equal(first.receipt, await readFile(resolve(root, "verification/cscl-10-sales-candidate-receipt-v1.json"), "utf8"));
  assert.equal(first.guide, await readFile(resolve(root, "docs/architecture/cscl-10-sales-mapping-application-guide-v1.md"), "utf8"));
  ...
  assert.equal(receipt.outcome, "FROZEN_EMPTY_CORE_WITH_PROMOTED_VARIANT_RELATION_AND_SOURCE_BOUND_LIMITS");
  ...
  assert.match(first.guide, /variant relation/i);
});