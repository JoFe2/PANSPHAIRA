...
test("CCP-M1-INT-026 Verification DAG ownership expansion advances its graph version", () => {
  ...
  assert.equal(dag.graphVersion, 46);
  assert.equal(dag.nodes.length, 54);
  ...
  assert.equal(dag.nodes.filter(({ id }) => id === "ap-04-incoming-invoice-erv-v1").length, 1);
  assert.equal(dag.nodes.filter(({ id }) => id === "ap-05-incoming-invoice-adaptive-ui-v1").length, 1);
